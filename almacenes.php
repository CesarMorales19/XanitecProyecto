...
try {
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        if ($post_accion === 'agregar') {
            $stmt = $pdo->prepare("EXEC agregar_almacen :nombre, :ubicacion, :responsable");
            ...
            $stmt = $pdo->prepare("EXEC actualizar_almacen :id, :nombre, :ubicacion, :responsable");
...
if ($accion === 'eliminar' && $id > 0) {
    try {
        $stmt = $pdo->prepare("EXEC eliminar_almacen :id");
        $stmt->bindParam(':id', $id, PDO::PARAM_INT);
        $stmt->execute();
        ...
        echo "Error al eliminar almacén: " . $e->getMessage();
    }
}
